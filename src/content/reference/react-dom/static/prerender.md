...
`prerender`, [Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) பயன்படுத்தி React tree ஒன்றை static HTML string ஆக render செய்கிறது.
...
இந்த API [Web Streams](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API)-ஐ சார்ந்துள்ளது. Node.js-க்கு, இதற்கு பதிலாக [`prerenderToNodeStream`](/reference/react-dom/static/prerenderToNodeStream) பயன்படுத்தவும்.
...
* **optional** `options`: Static generation options கொண்ட object.
  ...
  * **optional** `onError`: [recoverable](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-outside-the-shell) ஆனதோ [அல்லாததோ](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-inside-the-shell), server error ஏற்படும் ஒவ்வொரு முறையும் fire ஆகும் callback. Default ஆக, இது `console.error` மட்டுமே call செய்கிறது. [Crash reports log செய்ய](/reference/react-dom/server/renderToReadableStream#logging-crashes-on-the-server) override செய்தால், இன்னும் `console.error` call செய்வதை உறுதிசெய்யவும். Shell emit ஆகும்முன் [status code adjust செய்யவும்](/reference/react-dom/server/renderToReadableStream#setting-the-status-code) இதைப் பயன்படுத்தலாம்.
...
- Rendering successful என்றால், Promise பின்வருவன கொண்ட object-க்கு resolve ஆகும்:
  - `prelude`: HTML-ன் [Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API). இந்த stream-ஐ chunks-ஆக response அனுப்ப பயன்படுத்தலாம், அல்லது முழு stream-ஐ string ஆக read செய்யலாம்.
  - `postponed`: `prerender` முடிவடையவில்லை என்றால் [`resume`](/reference/react-dom/server/resume)-க்கு pass செய்யக்கூடிய JSON-serializeable, opaque object. இல்லையெனில் `null`; அதாவது `prelude` எல்லா content-யையும் கொண்டுள்ளது, resume தேவையில்லை.
- Rendering fail ஆனால், Promise reject செய்யப்படும். [Fallback shell output செய்ய இதைப் பயன்படுத்தவும்.](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-inside-the-shell)
...
### நான் `prerender` எப்போது பயன்படுத்த வேண்டும்? {/*when-to-use-prerender*/}

Static `prerender` API, static server-side generation (SSG)-க்கு பயன்படுத்தப்படுகிறது. `renderToString`-க்கு மாறாக, `prerender` resolve ஆகும்முன் எல்லா data load ஆக காத்திருக்கும். Suspense பயன்படுத்தி fetch செய்ய வேண்டிய data உட்பட, full page-க்கான static HTML generate செய்ய இது பொருத்தமானது. Content load ஆகும் போதே stream செய்ய, [renderToReadableStream](/reference/react-dom/server/renderToReadableStream) போன்ற streaming server-side render (SSR) API பயன்படுத்தவும்.

Partial pre-rendering support செய்ய, `prerender` abort செய்யப்பட்டு பின்னர் `resumeAndPrerender` மூலம் continued செய்யப்படவோ அல்லது `resume` மூலம் resumed செய்யப்படவோ முடியும்.
...
உங்கள் React tree-ஐ static HTML ஆக [Readable Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream)-க்குள் render செய்ய `prerender` call செய்யவும்:
...
இதை [`resume`](/reference/react-dom/server/resume) அல்லது [`resumeAndPrerender`](/reference/react-dom/static/resumeAndPrerender) உடன் சேர்த்து partial prerendering-க்கு பயன்படுத்தலாம்.
...
Content load ஆகும் போதே stream செய்ய, [renderToReadableStream](/reference/react-dom/server/renderToReadableStream) போன்ற streaming server render API பயன்படுத்தவும்.
